#!/usr/bin/env python3
"""
Knowledge Graph Extraction Pipeline (refactored)
...
@dataclass
class Batch:
    """Container for a processed batch."""
...
class CustomDataLoader:
    ...
    def create_batch_instructions(self, batch_data: List[Dict[str, Any]]) -> Dict[str, List[List[Dict[str, str]]]]:
    ...
    def __iter__(self):
        ...
        for i in tqdm(range(start_idx, len(self.processed_data), batch_size)):
            ...
            yield Batch(messages=instructions, ids=batch_ids, texts=batch_texts, metadata=batch_metadata)
...
class KnowledgeGraphExtractor:
    ...
    def process_stage(self, instructions: Dict[str, str], result_schema: Dict) -> Tuple[List[str], List[List[Dict[str, Any]]]]:
        """Process a stage of triple extraction."""
    ...
    def run_extraction(self):
        """Run the complete knowledge graph extraction pipeline (refactored)."""
        os.makedirs(f"{self.config.output_directory}/kg_extraction", exist_ok=True)
        ...
        batch_counter = 0
        extraction_start_time = time.time() if self.config.record else None
        
        with torch.no_grad(), open(output_file, "w") as output_stream:
            for batch in data_loader:
                batch_counter += 1
                self._process_batch(batch, output_stream)
        ...
        if self.config.record and self.config.benchmark:
            self._append_total_time(output_file, extraction_start_time)

    def _process_batch(self, batch: Batch, output_stream: TextIOWrapper):
        stage_outputs = self._run_stages(batch.messages)

        for result in self._yield_sample_results(batch, stage_outputs):
            if self.config.debug_mode:
                self.debug_print_result(result)
            output_stream.write(json.dumps(result, ensure_ascii=False) + "\n")
            output_stream.flush()

    def _run_stages(self, messages_dict: Dict[str, List[List[Dict[str, str]]]]) -> Dict[str, Tuple[List[str], List[List[Dict[str, Any]]]]]:
        stage_outputs: Dict[str, Tuple[List[str], List[List[Dict[str, Any]]]]] = {}
        for key, msgs in messages_dict.items():
            batch_result = self.process_stage(msgs, result_schema=self.result_schema[key])
            stage_outputs[key] = batch_result
        return stage_outputs

    def _yield_sample_results(self, batch: Batch, stage_outputs: Dict[str, Tuple[List[str], List[List[Dict[str, Any]]]]]):
        result_keys = list(stage_outputs.keys())
        for idx, (sample_id, text, metadata) in enumerate(zip(batch.ids, batch.texts, batch.metadata)):
            llm_outputs, triple_dicts = [], []
            for key in result_keys:
                llm_output, triple_dict = stage_outputs[key]
                llm_outputs.append(llm_output[idx])
                triple_dicts.append(triple_dict[idx])
            yield self.prepare_result_dict(sample_id, text, metadata, (llm_outputs, triple_dicts), result_keys)

    def _append_total_time(self, output_file, start_time):
        ...
        try:
            ...
            last_json["total_extraction_time_seconds"] = total_time
